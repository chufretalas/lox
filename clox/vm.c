...
#define READ_CONSTANT_FROM(index)                                              \
...
#define READ_STRING_LONG() AS_STRING(READ_CONSTANT_FROM(READ_24BIT()))
    ...
    for (;;) {
        ...
        case OP_SET_PROPERTY_LONG: {
            ...
            tableSet(&instance->fields,
                     instruction == OP_SET_PROPERTY ? READ_STRING()
                                                    : READ_STRING_LONG(),
                     peek(0));
            Value value = pop();
            pop();
        ...
        case OP_CLOSURE_LONG: {
            ObjFunction *function = AS_FUNCTION(READ_CONSTANT_FROM(READ_24BIT()));
            ObjClosure *closure = newClosure(function);
            push(OBJ_VAL(closure));
            for (int i = 0; i < closure->upvalueCount; i++) {
                uint8_t isLocal = READ_BYTE();
